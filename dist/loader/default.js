...
mw.hook('WikiForum').add(function (Core) {
  ...
  if (typeof loadNS === 'number') [loadNS], _readOnlyError("loadNS");
  if (loadNS.length < 1 && conf.wgNamespaceIds.forum) [conf.wgNamespaceIds.forum], _readOnlyError("loadNS");

  if (loadNS.includes(conf.wgNamespaceNumber) && // $('.wiki-forum').length > 0 &&
  conf.wgArticleId !== 0 && conf.wgAction === 'view') {
